    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-6">
        <div className="flex items-center gap-3">
          <div className={`relative flex items-center justify-center bg-white rounded-xl p-1.5 shadow-sm overflow-hidden
          ...
          {!collapsed && <div className="font-bold text-lg text-zinc-100/90 whitespace-nowrap tracking-tight">Web3 Wizard</div>}
        </div>
        ...
        {links.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <div key="collabs" className="mb-2">
                <button
                  ...
                  title="Collabs"
                  className={`w-full flex items-center ${collapsed ? "justify-center p-2" : "gap-3.5 px-4 py-3"} rounded-lg transition-all duration-200 group
                    ${activeRoot 
                      ? "bg-white text-zinc-900" 
                      : "text-zinc-500 hover:bg-zinc-700/30 hover:text-zinc-300"}`}
                >
                  <Icon size={collapsed ? 22 : 20} className={`transition-colors ${activeRoot ? "text-zinc-900" : "group-hover:text-zinc-300"}`} />
                  {!collapsed && <span className="font-medium text-[15px]">Collabs</span>}
                </button>

                {/* Submenu */}
                {!collapsed && collabsOpen && (
                  <div className="mt-2 ml-5 pl-4 border-l border-zinc-700/50 space-y-1">
                    <Link
                      ...
                      className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] transition-all
                        ${activeRoot && tabParam === "ongoing"
                          ? "text-zinc-100 bg-zinc-700/40 font-medium"
                          : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700/20"}`}
                    >
                      <Loading02Icon size={15} className={activeRoot && tabParam === "ongoing" ? "animate-spin text-indigo-400" : ""} />
                      <span>Ongoing</span>
                    </Link>
                    <Link
                      ...
                      className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] transition-all
                        ${activeRoot && tabParam === "done"
                          ...
                          : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700/20"}`}
                    >
                      <CheckmarkCircle01Icon size={15} className={activeRoot && tabParam === "done" ? "text-emerald-400" : ""} />
                      <span>Done</span>
                    </Link>
                  </div>
                )}
          ...
          return (
            <Link
              ...
              href={href}
              title={label}
              className={`flex items-center ${collapsed ? "justify-center p-2" : "gap-3.5 px-4 py-3"} rounded-lg transition-all duration-200 group
                  ${active 
                  ? "bg-white text-zinc-900 font-medium" 
                  : "text-zinc-500 hover:bg-zinc-700/30 hover:text-zinc-300 font-normal"}`}
            >
              <Icon size={collapsed ? 22 : 20} className={`transition-colors ${active ? "text-zinc-900" : "group-hover:text-zinc-300"}`} />
      ...
      <div className="px-4 py-6 border-t border-zinc-800/60 bg-black/20">
        <button
          ...
          title="Logout"
        >
          <Logout03Icon size={collapsed ? 24 : 20} className="group-hover:text-zinc-200 transition-colors" />
